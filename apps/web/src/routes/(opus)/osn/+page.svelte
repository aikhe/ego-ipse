<script lang="ts">
  ...
  import { sizedOsnUrl } from '$lib/sanity/opusOsn';
  import type { PageProps } from './$types';

  let { data }: PageProps = $props();

  // sanity-only: no fallback copy. missing description renders nothing.
  const osnParagraphs = $derived(
    data.sanityOsn?.description
      ? splitDescriptionParagraphs(data.sanityOsn.description)
      : []
  );
  const osnImages = $derived(data.sanityOsn?.images ?? []);
  const figCount = $derived(osnImages.length);

  const figLabel = (i: number) => `Fig ${String(i + 1).padStart(2, '0')}`;

  // left index column mirrors the right content heights so 01 sits level
  // with the intro and each fig 01 sits level with its figure
  // (same pattern as the fleur page, minus the extras section).
...
<div class="opus-canvas">
  <div class="opus-grid">
    ...
    <div class="opus-col opus-col--2" aria-hidden="true">
      ...
      {#each osnImages as image, i (image.src ?? i)}
        <div
          class="opus-col__section opus-col__section--osn-fig"
    ...
    <div class="opus-col opus-col--3" bind:this={rightColEl}>
      <div
        ...
        aria-hidden="true"
      ></div>
      <div class="opus-section opus-section--osn opus-section--osn-part">
        <h2 class="opus-osn">Osn</h2>
        {#each osnParagraphs as para, k (k)}
          <p class="opus-osn__desc">
      ...
      {#each osnImages as image, i (image.src ?? i)}
        <figure
          class="opus-section opus-section--osn-fig opus-section--osn-part"
          aria-label={image.alt ?? `Osn figure ${i + 1}`}
        >
          {#if image.src}
            <img
              class="opus-osn__img"
              src={sizedOsnUrl(image.src)}
              alt={image.alt ?? `Osn figure ${i + 1}`}
...
<style>
  ...
  .opus-col__section--osn-fig {
    margin-top: 0.5rem;
  }

  .opus-col__section--01 + .opus-col__section--osn-fig {
    margin-top: 2rem;
  }
