...
## Intent
Maintain a clean, technical, and descriptive commit/PR history following a strict one-line and structured format.

## Rules
- MUST ONLY commit when explicitly instructed by the USER.
- MUST NOT stage or commit the `.agent` directory.
- MUST use a single-line commit message format.
...
- MUST wrap long messages immediately after a `+` or `&`.
