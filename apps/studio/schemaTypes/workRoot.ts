...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      name: 'width',
      type: 'number',
      description: 'Image cover width on hover card (e.g. 340)',
      validation: (Rule) => Rule.required().error('Width is required'),
    }),
    defineField({
      title: 'Height',
      name: 'height',
      type: 'number',
      description: 'Image cover height on hover card (e.g. 440)',
      validation: (Rule) => Rule.required().error('Height is required'),
    }),
    defineField({
      title: 'Content',
      name: 'content',
